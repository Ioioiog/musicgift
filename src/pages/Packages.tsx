...
const Packages = () => {
  ...
  return <div className="min-h-screen bg-gradient-to-br from-[#0D1117] via-[#161B22] to-[#0D1117]">
      {/* Hero Section */}
      ...
      <section className="py-16 px-4 relative">
        <div className="container mx-auto">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {packages.map((pkg, index) => <Card key={index} className="glass-card hover:translate-y-[-4px] transition-all duration-300 border-dark-border/50">
                <CardHeader>
                  <div className="flex items-center justify-between mb-4">
                    {pkg.icon}
                    <Badge variant="secondary" className="text-primary bg-slate-50 py-[8px]">
                      {pkg.price}
                    </Badge>
                  </div>
                ...
                <CardContent>
                  <ul className="space-y-3">
                    {pkg.features.map((feature, i) => <li key={i} className="flex items-start gap-3">
                ...
                <CardFooter className="flex flex-col items-stretch gap-4 pt-6">
                  ...
                  <Button className="w-full bg-gradient-to-r from-primary to-accent hover:opacity-90 text-white font-medium" onClick={handleOrderClick}>
                    Alege acest pachet
                  </Button>
                </CardFooter>
              </Card>)}
      ...
      <section className="py-16 px-4 relative">
        <div className="container mx-auto max-w-4xl">
          ...
          <div className="grid gap-6">
            {addOns.map((addon, index) => <Card key={index} className="glass-card hover:translate-y-[-2px] transition-all duration-300 border-dark-border/50">
      ...
      <section className="py-20 px-4">
        <div className="container mx-auto max-w-2xl text-center">
          ...
          <Button size="lg" className="bg-gradient-to-r from-primary to-accent hover:opacity-90 text-white px-8 py-6 text-lg font-medium" onClick={handleOrderClick}>
